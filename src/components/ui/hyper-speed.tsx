...
const distortions: Distortions = {
  ...
  turbulentDistortion: {
    ...
    getJS: (progress: number, time: number) => {
      ...
      const camProgress = 0.0125; // Match the GLSL shader's fixed reference point
      const distortion = new THREE.Vector3(
        getX(progress) - getX(camProgress),
        getY(progress) - getY(camProgress),
        0
      );
...
class App {
  ...
  update(delta: number) {
    ...
    const time = (this.clock.elapsedTime + this.timeOffset) * speed;
    ...
    if (typeof this.options.distortion === 'object' && this.options.distortion.getJS) {
      ...
      const sway = this.options.cameraSway ?? 1;
      let swayX = distortion.x * sway;
      let swayY = distortion.y * sway;

      // Clamp the sway vector to a circular pixel-space bound so the
      // vanishing point never drifts more than maxSwayPixels from center
      const maxPx = this.options.maxSwayPixels;
      if (maxPx !== undefined && maxPx > 0) {
        const lookAtDistance = Math.abs(distortion.z);
        const fovRad = THREE.MathUtils.degToRad(this.camera.fov);
        const viewportHeight = this.renderer.domElement.clientHeight || 1;
        const maxWorldOffset =
          (maxPx * 2 * lookAtDistance * Math.tan(fovRad / 2)) / viewportHeight;

        const magnitude = Math.sqrt(swayX * swayX + swayY * swayY);
        if (magnitude > maxWorldOffset) {
          const scale = maxWorldOffset / magnitude;
          swayX *= scale;
          swayY *= scale;
        }
      }
